...
def non_max_suppression(prediction, conf_thres=0.5, nms_thres=0.4):
    ...
    nms_thres以上の矩形を切るので厳しくしたかったら低くする
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        # print(image_pred[image_pred[:, 4] >= conf_thres])
        ...
        class_confs, class_preds = image_pred[:, 5:].max(1, keepdim=True) #maxはindicesも返す
...
def bbox_on_img(img, detection, category_id_to_name, BOX_COLOR=(255, 0, 0), TEXT_COLOR = (255, 255, 255), thickness=2):
    """
    args
        detection(detect_number * 7 )
            array([[x1, y1, x2, y2, object_conf, class_score, class_pred], ..])
    """
    for i in range(len(detection)):
        bbox = detection[i,:4]
        # print(score)
        score = round(float(detection[i,5]), 3)
        class_id = int(detection[i,6])
        ...
        cv2.putText(img, text, (x_min, y_min - int(0.3 * text_height)), cv2.FONT_HERSHEY_SIMPLEX, 0.35, TEXT_COLOR, lineType=cv2.LINE_AA)
    return img
